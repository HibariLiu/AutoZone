...
def get_apptask_times(singel_task_date):
    if singel_task_date != '':
...
def get_task_stepdata():
    ...
    for one_result in get_selectresult:
        ...
        run_appcasestep_intask(case_list,case_count)
        # readSQLCounts()
...
#将步骤中的参数读取出来
def run_appcasestep_intask(case_list,case_count):
...
def write_name_txt():
    ...
    content3 = "*** Variables ***\n"
    content6 = "${picture_path}    F:/AutoTestPlat/AutoTestPlat/webtest/media\n"
    content4 = "*** Test Cases ***\n"
    txtfile = open(project_path, "a+")
    txtfile.writelines([content1,"\r",content7,"\r",content5,"\r",content8,"\r",content3,"\r",content6,"\r", content4,"\r"])
    time.sleep(2)
    txtfile.close()
    print("主体写入完成")
