...
def readappcaseSQL(upperlevel_id):
    sql = "SELECT apptestlocation,appfindmethod,appkwargesone,appkwargestwo,appkwargesthree,appkwargesfour,appassertdata from apptest_appcasestep where Appcase_id="+upperlevel_id
    coon = pymysql.connect(user='root', password='test123456', db='autotest', port=3306, host='127.0.0.1',
                           charset='utf8')
    cursor = coon.cursor()
    appcasestepcheck = cursor.execute(sql)  #读取页面上的执行步骤
    get_selectresult = cursor.fetchmany(appcasestepcheck)  #获取所有的查询结果
    case_count = 0
...
def readSQLCounts(upperlevel_id):
    sql2 = "SELECT count(*)from apptest_appcasestep where Appcase_id=" + upperlevel_id
    coon = pymysql.connect(user='root', password='test123456', db='autotest', port=3306, host='127.0.0.1',
                           charset='utf8')
    cursor = coon.cursor()
...
#将数据库中的数据取出并写入到txt中
def test_appcase_step(upperlevel_id,case_list,case_count):
    ...
    for case in case_list:
        try:
            apptestlocation = case[0]
            appfindmethod = case[1]
            appkwargesone = case[2]
            appkwargestwo = case[3]
            appkwargesthree = case[4]
            appkwargesfour = case[6]
            appassertdata = case[7]
    ...
    try:
        if appkwargesone =="":
            txtfile.write("\t"+appfindmethod+"\n")
        elif appkwargestwo =="":
            txtfile.write("\t"+appfindmethod+"\t"+appkwargesone+"\n")
        elif appkwargesthree =="":
            txtfile.write("\t"+appfindmethod+"\t"+appkwargesone+"\t"+appkwargestwo+"\n")
        elif appkwargesfour =="":
            txtfile.write("\t"+appfindmethod+"\t"+appkwargesone+"\t"+appkwargestwo+"\t"+appkwargesthree+"\n")
        else:
            txtfile.write("\t"+appfindmethod+"\t"+appkwargesone+"\t"+appkwargestwo+"\t"+appkwargesthree+"\t"+appkwargesfour+"\n")
    except Exception as e:
...
def write_to_txt(upperlevel_id):
    ...
    content3 = "*** Variables ***\n"
    content6 = "${picture_path}    F:/AutoTestPlat/AutoTestPlat/apptest/media\n"
    content4 = "*** Test Cases ***\n"
    txtfile = open(project_path, "a")
...
# 移除指定路径下的txt文件
def remove_apptest_txt(txtname):
    number = str(txtname)
    robotcasename = "test00" + number + ".txt"
    path = os.path.abspath(os.path.dirname(__file__))  # 获取当前工程目录
    project_path = os.path.join(path, robotcasename)  # 加入完整的目录和文件名称
